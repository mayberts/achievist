...
async def ea_debug(type: str | None = Query(None)):
    ...
    from app.platforms.ea import _GRAPHQL

    pool = await db.get_pool()
    async with pool.connection() as conn:
        row = await _fetchrow(
            conn, "SELECT credentials FROM linked_accounts WHERE platform = 'ea' LIMIT 1"
        )
    if not row or not (row["credentials"] or {}).get("access_token"):
        return {"error": "No EA account connected with an access token."}
    token = row["credentials"]["access_token"]
