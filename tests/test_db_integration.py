...
async def test_upsert_account_creates_and_updates(db_conn):
    id1 = await db.upsert_account(db_conn, "steam", "111", {"api_key": "a"})
    id2 = await db.upsert_account(db_conn, "steam", "111", {"api_key": "b"})
    assert id1 == id2  # same (platform, external_id) -> same row, credentials updated

    row = await db.get_account(db_conn, id1)
    assert row["credentials"] == {"api_key": "b"}


async def test_reconnect_under_a_different_external_id_leaves_an_orphan_without_cleanup(db_conn):
    ...
    # because the upsert's conflict target is (platform, external_id).
    await db.upsert_account(db_conn, "ubisoft", "profile-1", {"ticket": "a"})
    await db.upsert_account(db_conn, "ubisoft", "profile-2", {"ticket": "b"})

    rows = await db.list_accounts(db_conn)
    assert len(rows) == 2


async def test_delete_other_accounts_for_platform_cleans_up_the_orphan(db_conn):
    await db.upsert_account(db_conn, "ubisoft", "profile-1", {"ticket": "a"})
    await db.upsert_account(db_conn, "ubisoft", "profile-2", {"ticket": "b"})

    await db.delete_other_accounts_for_platform(db_conn, "ubisoft", "profile-2")
...
async def test_delete_other_accounts_for_platform_is_scoped_to_one_platform(db_conn):
    await db.upsert_account(db_conn, "steam", "111", {})
    await db.upsert_account(db_conn, "ubisoft", "profile-1", {})
    await db.upsert_account(db_conn, "ubisoft", "profile-2", {})
...
async def test_schema_migration_dedupes_pre_existing_orphan_rows(db_conn):
    ...
    # everything but the most-recently-created row per platform.
    ...
    rows_before = await db.list_accounts(db_conn)
    ...
    rows_after = await db.list_accounts(db_conn)
...
async def test_unlocks_since_only_returns_unlocks_after_the_cutoff(db_conn):
...
async def test_unlocks_since_ignores_locked_achievements(db_conn):
    account_id = await db.upsert_account(db_conn, "steam", "111", {})
...
async def test_unlocks_since_is_scoped_to_the_account(db_conn):
    account_a = await db.upsert_account(db_conn, "steam", "111", {})
    account_b = await db.upsert_account(db_conn, "steam", "222", {})
    game_id = await db.upsert_platform_game(db_conn, "steam", "42", "Some Game", None, 1)
